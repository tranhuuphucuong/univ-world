    ...
    <div className="pt-40">
      <Container>
        <FeatureIconContainer className="flex justify-center items-center overflow-hidden">
          <IconReceipt2 className="h-6 w-6 text-typo" />
...
const Card = ({ plan, onClick }: { plan: Plan; onClick: () => void }) => {
  return (
    <div
      className={cn(
        "p-4 md:p-4 rounded-3xl bg-base border-2 border-neutral-800",
        plan.featured && "border-neutral-50 bg-neutral-100"
      )}
    >
      <div
        className={cn(
          "p-4 bg-dim rounded-2xl shadow-[0px_-1px_0px_0px_var(--neutral-700)]",
      ...
      >
        <div className="flex justify-between items-center">
          ...
          {plan.featured && (
            <div
              className={cn(
                "font-medium text-xs px-3 py-1 rounded-full relative bg-base"
              )}
            >
        ...
        <Button
          ...
          className={cn(
            "w-full mt-10 mb-4",
            plan.featured &&
              "bg-black text-typo hover:bg-black/80 hover:text-typo"
          )}
          onClick={onClick}
        >
          {plan?.CTA?.text}
...
const Divider = ({ featured }: { featured?: boolean }) => {
  return (
    <div className="relative">
      ...
      />
      <div className={cn("w-full h-px bg-dim", featured && "bg-neutral-200")} />
      <div
        className={cn(
          "absolute inset-0 h-5 w-5 m-auto rounded-xl bg-dim shadow-[0px_-1px_0px_0px_var(--neutral-700)] flex items-center justify-center",
